  ...
  onStart: async function ({ event, api, args, message, chatId, userId, ctx }) {
    ...
    if (!botIsAdmin) {
      return message.reply(
        "❌ Bot needs admin rights to change group name."
      );
    }
    ...
    if (newName.length > 128) {
      return message.reply(
        "❌ Group name is too long! Maximum 128 characters."
      );
    }
    ...
    const keyboard = message.Markup.inlineKeyboard([
      [
        message.Markup.button.callback(
          "✅ Confirm",
          `confirm_gcname_${chatId}`
        ),
        message.Markup.button.callback(
          "❌ Cancel",
          `cancel_gcname_${chatId}`
        )
      ]
    ]);
      ...
      try {
        ...
        if (!botMember) {
          ...
          await ctx.editMessageText(
            "❌ Bot lost admin rights! Cannot change group name."
          );
        ...
        await ctx.editMessageText(
          `❌ Error changing group name: ${error.message}`
        );
