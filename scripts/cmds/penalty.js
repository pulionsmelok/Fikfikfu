...
module.exports = {
  ...
  onStart: async function ({ args, message, event, usersData, api }) {
    try {
      ...
      const imagePath = await sauvegarderImageDansCache(image);

      const sent = await api.sendPhoto(event.threadID, imagePath, {
  ...
  onCallback: async function ({ event, api, message, args }) {
    try {
      ...
      const imagePath = await sauvegarderImageDansCache(resultImage);
      ...
      await api.sendPhoto(event.threadID, imagePath, {
