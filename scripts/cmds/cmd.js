...
module.exports = {
	...
	onStart: async ({
		...
		// --- CHECK PENDING RENAME RESPONSE ---
		const pendingRename = global.cmdRenamePending?.get(String(event.senderID));
		if (pendingRename) {
			const newFileName = normalizeCommandFileName(args[0] || event.body);
			global.cmdRenamePending.delete(String(event.senderID));
			if (!newFileName) {
				return message.reply("⚠️ | Invalid new file name.");
			}
			const oldFilePath = commandPath(pendingRename.oldFileName);
			const newFilePath = commandPath(newFileName);

			if (fs.existsSync(newFilePath)) {
				return message.reply(`⚠️ | Command file "${newFileName}" already exists.`);
			}

			try {
				try {
					unloadScripts("cmds", pendingRename.oldFileName.slice(0, -3), configCommands, getLang);
				} catch (_) {}

				if (fs.existsSync(oldFilePath)) {
					fs.renameSync(oldFilePath, newFilePath);
				} else {
					fs.writeFileSync(newFilePath, pendingRename.rawCode, "utf8");
				}
				...
				const infoLoad = loadScripts(
					...
					getLang,
					fs.existsSync(newFilePath) ? undefined : pendingRename.rawCode
				);

				if (infoLoad.status !== "success") throw infoLoad.error;
				global.cmdInstallPending?.delete(`${event.senderID}:${pendingRename.oldFileName}`);
				return message.reply(`✅ | Renamed and installed command "${newFileName}" successfully!`);
			} catch (err) {
				return message.reply(`❌ | Failed to rename/install command:\n${err.message || err}`);
			}
		}
		...
		if (
			telegramDocument &&
			(String(commandName || "").toLowerCase() === "cmd" || String(commandName || "").toLowerCase() === "ck") &&
			...
			try {
				...
				if (fs.existsSync(commandPath(fileName))) {
					...
					return message.reply({
						...
						reply_markup: commandButtons(fileName, String(event.senderID)).reply_markup
					});
				}
				...
				return infoLoad.status === "success"
					? message.reply(
						getLang(
							"installed",
							infoLoad.name,
							path.join(__dirname, `${infoLoad.name}.js`).replace(process.cwd(), "")
						)
					)
					: message.reply(
						getLang(
		...
		========================= */
			...
			try {
				const infoUnload = unloadScripts(
					"cmds",
		...
		========================= */
			...
			if (!oldFile || !newFile)
				return message.reply("⚠️ | Usage: cmd rename old.js new.js");

			if (!fs.existsSync(commandPath(oldFile)))
				return message.reply(getLang("missingFile", oldFile));

			if (fs.existsSync(commandPath(newFile)))
				return message.reply(`⚠️ | Command file "${newFile}" already exists`);

			try {
				try {
					unloadScripts("cmds", oldFile.slice(0, -3), configCommands, getLang);
				} catch (_) {}
		...
		========================= */
			...
			if (!args[1])
				return message.reply("⚠️ | Please enter the command file name you want to delete");
	...
	onCallback: async function ({ event, ctx, message, api, getLang }) {
		...
		if (!fileName) {
			await ctx.answerCbQuery("❌ Invalid file name.");
			return;
		}
		...
		if (action === "replace") {
			try {
				const db = global.db || {};
				try {
					global.utils.unloadScripts("cmds", fileName.slice(0, -3), configCommands, getLang);
				} catch (_) {}
				...
				const infoLoad = loadScripts(
					...
					log,
					configCommands,
					global.GoatBot.api || global.GoatBot,
					db.threadModel,
					db.userModel,
					db.dashBoardModel,
					db.globalModel,
		...
		if (action === "rename") {
			...
			global.cmdRenamePending.set(String(userID), {
				...pending,
...
function loadScripts(
	...
	try {
		...
		if (!oldCommandName) {
			throw new Error(`Name of command is missing in "${removeHomeDir(pathCommand)}"`);
		}
		...
		const handlerNames = ["onStart", "onChat", "onReply", "onReaction", "onEvent", "onAnyEvent", "onFirstChat", "onLoad"];
		...
		if (command.onFirstChat) allOnFirstChat.push({
