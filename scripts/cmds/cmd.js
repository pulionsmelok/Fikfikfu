...
module.exports = {
	config: {
		...
		role: 2,
		usePrefix: true,
		description: { vi: "Quản lý các tệp lệnh của bạn", en: "Manage your command files", bn: "Manage আপনার কমান্ড ফাইলগুলো" },
		category: "owner",
		guide: {
	...
	onCallback: async function ({ event, ctx, message, api, getLang }) {
		...
		if (action === "replace") {
			try {
				...
				const infoLoad = loadScripts("cmds", fileName, log, configCommands, global.GoatBot.api || global.GoatBot, db.threadModel, db.userModel, db.dashBoardModel, db.globalModel, db.threadsData, db.usersData, db.dashBoardData, db.globalData, getLang, pending.rawCode);
				if (infoLoad.status !== "success") throw infoLoad.error;
				global.cmdInstallPending.delete(key);
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
	...
	try {
		...
		if (allPackage) {
			allPackage = allPackage.map(p => p.match(/[`'"]([^`'"]+)[`'"]/)[1]).filter(p => !p.startsWith("/") && !p.startsWith("./") && !p.startsWith("../") && !p.startsWith(__dirname));
			for (let packageName of allPackage) {
				...
				if (!packageAlready.includes(packageName)) {
					...
					if (!fs.existsSync(`${process.cwd()}/node_modules/${packageName}`)) {
						let waiting;
						try {
							waiting = setInterval(() => { count++; loading.info("PACKAGE", `Installing ${packageName} ${spinner[count % spinner.length]}`); }, 80);
		...
		if (!oldCommandName) throw new Error(`Name of command is missing in "${removeHomeDir(pathCommand)}"` );
		...
		const handlerNames = ["onStart", "onChat", "onReply", "onEvent", "onAnyEvent", "onFirstChat", "onLoad"];
