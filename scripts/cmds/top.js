...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData, message }) {
    ...
    for (let i = 0; i < topUsers.length; i++) {
      ...
      try {
        let url;
        if (typeof api.getAvatarUrl === "function") {
          url = await api.getAvatarUrl(user.userID);
        } else if (typeof api.getUserProfilePhotos === "function" && typeof api.getFileLink === "function") {
          const photos = await api.getUserProfilePhotos(Number(user.userID), { limit: 1 });
          if (photos.total_count > 0) {
            const sizes = photos.photos[0];
            url = await api.getFileLink(sizes[sizes.length - 1].file_id);
          }
        }
        if (url) {
    ...
    const result = await api.sendPhoto(event.threadID, fs.createReadStream(imagePath), {
      caption: "🏆 leaderboard",
