...
module.exports = {
  config: {
    ...
    usePrefix: true,
    shortDescription: "",
    longDescription: "Download media from a direct link",
    category: "media",
    guide: "{pn} <link>"
  },
  ...
  onStart: async function ({ api, event, args, chatId, message }) {
    ...
    try {
      ...
      if (extension === ".jpg" || extension === ".jpeg" || extension === ".png") {
        await api.sendPhoto(chatId || event.threadID, filePath, opts);
      } else if (extension === ".mp4") {
        await api.sendVideo(chatId || event.threadID, filePath, opts);
      } else if (extension === ".mp3" || extension === ".wav") {
        await api.sendAudio(chatId || event.threadID, filePath, opts);
      } else {
        await api.sendDocument(chatId || event.threadID, filePath, opts);
      }
    } catch (error) {
