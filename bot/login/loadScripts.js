	...
	for (const folderModules of folders) {
		...
		for (const file of Files) {
			...
			try {
				...
				if (configCommand.countDown === undefined && configCommand.cooldown !== undefined)
					configCommand.countDown = configCommand.cooldown;
				...
				for (const handlerName of [
					...
					"onReply", "reply", "onChat", "chat", "handleEvent", "noPrefix"
				]) {
				...
				const { onFirstChat, onChat, onLoad, onEvent, onAnyEvent, handleEvent, noPrefix } = command;
				...
				if (typeof command.chat === "function" && !GoatBot.chat.includes(commandName))
					GoatBot.chat.push(commandName);
				if (typeof handleEvent === "function" && !GoatBot.handleEvent.includes(commandName))
					GoatBot.handleEvent.push(commandName);
				if (typeof noPrefix === "function" && !GoatBot.noPrefix.includes(commandName))
					GoatBot.noPrefix.push(commandName);
