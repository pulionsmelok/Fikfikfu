...
try {
	const prefixModePath = path.join(process.cwd(), "scripts/cmds/S1DD1K/prefixmode.json");
...
async function onStart() {
			...
			let getCoolDown = command.config.countDown ?? command.config.cooldown;
			...
			try {
				...
				const startHandler = command.onStart || command.start || command.run || command.prefix;
...
async function onReply() {
			...
			try {
				...
				const replyHandler = command.onReply || command.reply;
		...
		// ———————————————— LEGACY HANDLER ———————————————— //
async function runLegacyAlwaysHandler(listName, handlerName, args = []) {
			const list = GoatBot[listName] || [];
			for (const key of list) {
				const command = GoatBot.commands.get(key);
				if (!command || typeof command[handlerName] !== "function") continue;
				const commandName = command.config.name;
				const roleConfig = getRoleConfig(utils, command, isGroup, threadData, commandName);
				const roleKey = handlerName === "handleEvent" ? "onChat" : handlerName;
				if ((roleConfig[roleKey] ?? 0) > role) continue;
				const getText2 = createGetText2(langCode, `${process.cwd()}/languages/cmds/${langCode}.js`, prefix, command);
				try {
					await command[handlerName].call(command, {
						...parameters, args, commandName, getLang: getText2,
						isUserCallCommand
					});
					log.info(handlerName, `${commandName} | ${senderID} | ${threadID}`);
				}
				catch (err) {
					log.err(handlerName, `An error occurred when calling ${handlerName} for ${commandName}`, err);
				}
			}
		}

		// ———————————————— NO PREFIX ———————————————— //
async function onNoPrefix() {
			if (typeof body !== "string" || body.trim().startsWith(prefix)) return;
			const args = body.trim() ? body.trim().split(/ +/) : [];
			await runLegacyAlwaysHandler("noPrefix", "noPrefix", args);
		}

		// ———————————————— LEGACY CHAT ———————————————— //
async function onLegacyChat() {
			const args = body ? body.split(/ +/) : [];
			await runLegacyAlwaysHandler("chat", "chat", args);
		}

		// ———————————————— LEGACY HANDLE EVENT ———————————————— //
async function onLegacyHandleEvent() {
			const args = body ? body.split(/ +/) : [];
